...
<body>
  <h1><a href="${pageContext.request.contextPath}/">&lt;</a> 자유게시판</h1>
...
<label for="size" style="white-space: nowrap;">건수:</label>
  ...
  <div class="pagination">
    <c:if test="${pageResponse.prev}">
      <a href="freeBoard?pageNo=${pageResponse.startPage - 1}&size=${pageResponse.size}&searchValue=${pageResponse.searchValue}">«</a>
    </c:if>
    ...
    <c:forEach var="pageNo" begin="${pageResponse.startPage}" end="${pageResponse.endPage}">
      <c:choose>
        ...
        <c:otherwise>
          <a href="freeBoard?pageNo=${pageNo}&size=${pageResponse.size}&searchValue=${pageResponse.searchValue}">${pageNo}</a>
        </c:otherwise>
      </c:choose>
    </c:forEach>

    <c:if test="${pageResponse.next}">
      <a href="freeBoard?pageNo=${pageResponse.endPage + 1}&size=${pageResponse.size}&searchValue=${pageResponse.searchValue}">»</a>
    </c:if>
  </div>
  ...
  <script type="text/javascript">
    ...
    size.addEventListener("change", () => {
      location.href = "freeBoard?pageNo=1&size=" + size.value + "&searchValue=" + searchValue.value;
    });
  </script>
</body>
</html>
